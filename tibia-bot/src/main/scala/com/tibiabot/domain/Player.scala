 ...
 *
 *
 *  `tag` is a hunted-list label — see panels.ListTags. The column exists on both
 *  tables so one read path serves both; only the hunted panel offers it, because
 *  the tags describe an opponent.
