 ...
 *  breakdown.
 *
 *  Derived from method and URL rather than an application-side tag, because it is
 *  applied in JDA's HTTP client (see [[com.tibiabot.app.Bootstrap]]) where only
 *  the request is available — which is why it sees every call, including the
 *  unpaced death posts and command replies that never reach [[RateLimitedSender]].
 *
 *  Deliberately coarse and a closed set. Only five writes get a name, chiefly
 *  `PATCH message` (an online-list edit) against `PATCH channel` (a category
 *  rename): they dominate this bot's traffic and sit in completely different
 *  Discord buckets, so they must never share a row. Everything else becomes
 *  [[Other]] — a rounding error next to those, where one row per endpoint would
 *  bury the two that matter.
 *
 *  [[Other]] is recorded rather than discarded so this dimension keeps summing to
 *  the overall total; the dashboard leaves it out but the figures stay whole. */
