  ...
  private def handleNeutrals(event: SlashCommandInteractionEvent): Unit = {
    ...
    subCommand match {
      ...
      case "tag" =>
        ...
        val nameOption: String = options.getOrElse("name", "")
        val labelOption: String = options.getOrElse("label", "").replaceAll("[^a-zA-Z0-9\\s]", "")
        val emojiOption: String = options.getOrElse("emoji", "").trim
        if (toggleOption == "add") {
          if (labelOption == "" || emojiOption == ""){
            val embed = new EmbedBuilder().setDescription(s":x: You must supply a **label** and **emoji** when tagging a guild or player.").build()
            event.getHook.sendMessageEmbeds(embed).queue()
          } else {
