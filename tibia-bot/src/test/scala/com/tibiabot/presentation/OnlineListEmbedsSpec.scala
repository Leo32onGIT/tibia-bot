...
class OnlineListEmbedsSpec extends AnyFunSuite with Matchers {
  ...
  // --- the labels (27 Sep 2026) ---
  //
  // The list's headings are small grey labels now. They pack as the headings
  // they replaced did: allies, enemies and others open an embed, a guild's
  // stays with the lines before it — with a blank line above it, which the
  // headings did not need, since they came with their own space.

  private val allies = OnlineListGrouping.label("Allies", 2, ":ally:")
  private val enemies = OnlineListGrouping.label("Enemies", 1, ":enemy:")
  private val others = OnlineListGrouping.label("Others", 1)
  private val redRose = OnlineListGrouping.guildLabel("Red Rose", 2)

  test("an allies or enemies label opens an embed, with no gap above it") {
    OnlineListEmbeds.packMessages(List(allies, "a1", "a2", enemies, "e1")) shouldBe
      List(List(s"\n$allies\na1\na2", s"$enemies\ne1"))
  }

  test("a guild's label stays in the embed, a blank line standing it off the rows above") {
    OnlineListEmbeds.packMessages(List(enemies, "e1", redRose, "g1", "g2")) shouldBe
      List(List(s"\n$enemies\ne1\n\n$redRose\ng1\ng2"))
  }

  test("the others label opens an embed of its own, as the Others heading did") {
    OnlineListEmbeds.packMessages(List(redRose, "g1", "g2", others, "n1")) shouldBe
      List(List(s"\n$redRose\ng1\ng2", s"$others\nn1"))
  }

  test("a labelled roster stays inside both Discord caps, and the blank lines lose nothing") {
    def rows(from: Int, n: Int) = (from until from + n).map(i => s"$i" + ("r" * 139)).toList
    val lines =
      (OnlineListGrouping.label("Allies", 40, ":ally:") :: rows(0, 40)) :::
      (OnlineListGrouping.label("Enemies", 60, ":enemy:") :: rows(40, 60)) :::
      (1 to 8).toList.flatMap(g => OnlineListGrouping.guildLabel(s"Guild $g", 45) :: rows(100 + g * 45, 45))
    val packed = OnlineListEmbeds.packMessages(lines)
  ...
  test("a labelled list repacks stably: unchanged stays put, and a label is never stranded") {
    val lines = (1 to 30).toList.flatMap(g => OnlineListGrouping.guildLabel(s"Guild$g", 5) :: rows(5, from = g * 100))
    val posted = OnlineListEmbeds.packMessages(lines)
    OnlineListEmbeds.packMessagesStable(lines, posted) shouldBe posted
    ...
    (1 to 40).foreach { round =>
      current = current.patch(random.nextInt(current.size), rows(1, from = 70000 + round), 0)
      stable = OnlineListEmbeds.packMessagesStable(current, stable)
      withClue(s"round $round: ") {
        stable.flatMap(_.flatMap(_.split("\n").filter(_.nonEmpty).lastOption.filter(_.startsWith("-# ")))) shouldBe empty
      }
    }
    stable.flatMap(linesOf) shouldBe current
  }
}
