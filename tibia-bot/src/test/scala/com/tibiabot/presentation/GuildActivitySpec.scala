...
class GuildActivitySpec extends AnyFunSuite with Matchers {
  ...
  /** renameFromFormerNames reads the rows as a lookup — see
   *  [[com.tibiabot.state.ActivityIndexSpec]] for the lookup itself. */
  private def index(rows: PlayerCache*) = ActivityIndex(rows.toList)
  ...
  test("renameFromFormerNames matches the tracked row a character used to be stored under") {
    ...
    GuildActivity.renameFromFormerNames(ActivityIndex(activity), "Alice", List("Bob"), nobodyOnline) shouldBe
  ...
  test("renameFromFormerNames reports the guild the row is still recorded in, not the character's current one") {
    ...
    GuildActivity.renameFromFormerNames(ActivityIndex(activity), "Alice", List("Bob"), nobodyOnline).map(_.guild) shouldBe Some("Nemesis")
  }

  test("renameFromFormerNames is case-insensitive on the stored name") {
    val activity = List(row("bob"))
    GuildActivity.renameFromFormerNames(ActivityIndex(activity), "Alice", List("Bob"), nobodyOnline).map(_.oldName) shouldBe Some("bob")
  }

  test("renameFromFormerNames ignores a former name somebody else is online under") {
    val activity = List(row("Bob"))
    GuildActivity.renameFromFormerNames(ActivityIndex(activity), "Alice", List("Bob"), _ == "Bob") shouldBe None
  }
  ...
  test("renameFromFormerNames ignores a former name once the character is tracked under their new one") {
    ...
    GuildActivity.renameFromFormerNames(ActivityIndex(activity), "Alice", List("Bob"), nobodyOnline) shouldBe None
  }

  test("renameFromFormerNames ignores a character carrying its own name in former_names") {
    val activity = List(row("Bob"))
    GuildActivity.renameFromFormerNames(ActivityIndex(activity), "Alice", List("Bob", "alice"), nobodyOnline) shouldBe None
  }

  test("renameFromFormerNames returns nothing when no former name is tracked") {
    GuildActivity.renameFromFormerNames(index(row("Carol")), "Alice", List("Bob"), nobodyOnline) shouldBe None
    GuildActivity.renameFromFormerNames(index(row("Bob")), "Alice", Nil, nobodyOnline) shouldBe None
    GuildActivity.renameFromFormerNames(index(row("Bob")), "", List("Bob"), nobodyOnline) shouldBe None
  }
  ...
  test("a rename held back for the settling window is still there to find next poll") {
    val activity = List(row("Bob"))
    val first = GuildActivity.renameFromFormerNames(ActivityIndex(activity), "Alice", List("Bob"), nobodyOnline)
    ...
    GuildActivity.renameFromFormerNames(ActivityIndex(activity), "Alice", List("Bob"), nobodyOnline) shouldBe first
  ...
  test("once the row is moved a flip-flopping sheet cannot announce the rename again") {
    ...
    GuildActivity.renameFromFormerNames(ActivityIndex(moved), "Bob", Nil, nobodyOnline) shouldBe None
    // Then the fresh copy again — the moved row is what refuses it this time.
    GuildActivity.renameFromFormerNames(ActivityIndex(moved), "Alice", List("Bob"), nobodyOnline) shouldBe None
  }

  test("with two former names still tracked, the earlier former name wins") {
    // The list-scanning version returned whichever matching row came first in a
    // list with no meaningful order. Now it is the first former name that has a
    // row — the same answer whenever only one does, which is every ordinary
    // rename, and a stable one when a twice-renamed character left two behind.
    val activity = index(row("Carol", "Vindicate"), row("Bob", "Nemesis"))
    GuildActivity.renameFromFormerNames(activity, "Alice", List("Bob", "Carol"), nobodyOnline).map(_.oldName) shouldBe Some("Bob")
  ...
  test("a former name somebody is online under is skipped, not the whole rename") {
    ...
    val activity = index(row("Bob", "Nemesis"), row("Carol", "Vindicate"))
    GuildActivity.renameFromFormerNames(activity, "Alice", List("Bob", "Carol"), _ == "Bob").map(_.oldName) shouldBe Some("Carol")
  }
}
