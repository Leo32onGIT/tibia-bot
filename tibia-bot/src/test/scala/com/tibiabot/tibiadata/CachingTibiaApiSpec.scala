...
class CachingTibiaApiSpec extends AnyFunSuite with Matchers with JsonSupport {
  ...
  private def mkApi(stub: TibiaApi, cache: RedisCache,
    ...
    new CachingTibiaApi(stub, cache, boostedTtl, now)
  ...
  /** Stub TibiaApi counting calls per cached endpoint. */
  ...
  // --- the configured TTL is threaded through, not hardcoded ---

  test("cached endpoints store under the configured boosted TTL") {
    val stub = new StubApi(); val cache = new FakeCache()
    val api = mkApi(stub, cache, boostedTtl = 7.minutes)
    await(api.getBoostedBoss())
